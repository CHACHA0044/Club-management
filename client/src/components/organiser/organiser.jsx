...
const Organizer = () => {
  ...
  // ✅ Fixed: Get user from both storages and parse properly
  const getUserData = () => {
    try {
  ...
  // Mock events created by this organizer
  ...
  // Mock registrations for organizer's events
  ...
  useEffect(() => {
    // ✅ Fixed: Better role checking with debugging
    ...
    console.log("User is an organizer, loading data");
    setEvents(mockOrganizerEvents);
    setRegistrations(mockOrganizerRegistrations);
  ...
  const getPendingCount = (eventId) => {
    return registrations.filter(reg => reg.eventId === eventId && reg.status === "pending").length;
  };
